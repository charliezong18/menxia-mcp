...
**[门下](https://github.com/charliezong18/zhupi)的 agent 侧，做成 MCP server。**<br>
...
**另有一块早就上线且不属于 server**：拒绝对敕草仓裸 `gh pr create` 的[路由守卫](SPEC.zh-CN.md)。它住在 review-loop skill 里，刻意先上——理由见下面「MCP 治不了什么」。
...
**正中间那个仓就是全部接口。** 两侧谁也不调谁，各自只跟 GitHub 说话。所以门下随时可以摘掉，而这个 server 也可以换回 bash 脚本而人那边毫无察觉。
...
- **按个人尺度设计。** 一个评审人、一个敕草仓，没有多租户的打算。

## Fork 去用

敕草仓名和本地 checkout 路径是配置，不是常量：
