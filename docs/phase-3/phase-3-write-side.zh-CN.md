...
| 原来靠哪个 hook | 现在 |
|---|---|
| 已画可/已关的折不许回话（`guard-closed-folder`） | `reply_comment` 先 GET 一次查 state，非 open 直接拒 |
| inline 回话首行盖 `**回话**`（硬约定③） | 焊死：缺了自动补，同一行 |
| 不许发判（硬约定①） | 工具面上**根本没有这个能力** —— `commentId` 必填 |
