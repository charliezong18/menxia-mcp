...
拷入位置固定：`docs` 落 `docs/<basename>`，`assets` 落 `docs/assets/<basename>`——正文里的图片引用写 `](assets/foo.png)`，与现有仓内布局一致。
...
读完 `folder-lint.sh` 与 `open-folder.sh` 实现，**已确认三处文档与实现的漂移**——这是本条规矩的直接证据，移植时必须以下表的「实现真相」为准，另见 §5.3 待决。
...
| 4 | 图一起搬 | 正文引用的 `assets/**` 必须真在仓里 | 一致，但只匹配 markdown 的 `](assets/...)` 形式，**HTML `<img src>` 不查** |
...
有差异只有两种结局：是 bug 就修；是刻意改进（如 §5.1 表中 #1/#6/#4 的收紧）就写进本文档说清为什么。**对不齐不许上线。**
...
### 5.3 待决（移植时必须问 Charlie，不许自行决定）

- **#1 语言方向**：要不要真的加语言检测（英文版里若是中文则报错）？收紧了会不会误伤混排（代码块、术语表、专名）？
- **#6 五段缺项**：拦还是警告？文档和注释都说拦，代码说警告。
...
| **4** | 脚本退休；SKILL.md 瘦成「名词表 + 指向工具」；hook 改指 MCP | 收尾 |
