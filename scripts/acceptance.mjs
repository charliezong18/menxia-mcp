...
const main = async () => {
  ...
  check('counts.unanswered = 0', f17?.counts?.unanswered === 0, `实得 ${f17?.counts?.unanswered}`);
  ...
  check('counts.unanswered = 0（unknown 不谎报成待办）', f18?.counts?.unanswered === 0, `实得 ${f18?.counts?.unanswered}`);
  check('counts.unknown = 1', f18?.counts?.unknown === 1, `实得 ${f18?.counts?.unknown}`);
  ...
  check('每折都带 counts 三字段',
    open.folders?.every((f) => ['unanswered', 'unknown', 'inferred'].every((k) => typeof f.counts?.[k] === 'number')));
  ...
  for (const f of open.folders ?? []) {
    ...
    check(`#${f.number} counts 一致`, d && JSON.stringify(d.counts) === JSON.stringify(f.counts),
